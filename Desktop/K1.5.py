...
class RobotSim(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.draw_tool == TOOL_CIRCLE and self.circle_center is not None:
            ...
            qp.setFont(QFont("Arial", 11))
            qp.drawText(10, self.height() - 10,
                        f"Circle: r={r:.0f}px - drag to resize / release to commit")
        elif self.draw_tool == TOOL_CIRCLE:
            qp.setPen(QColor(255, 255, 100))
            qp.setFont(QFont("Arial", 11))
            qp.drawText(10, self.height() - 10, "Circle: click and drag to place")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        system_prompt = """You are Coordinate locator & planner where you plan locate & coordinates
first:
whenever an image is provided (which will be of a board), you will have to tell yourself internally (never output) in which column and row OBJECTS related to your below tasks is in.
eg: (column, row) or (E, 3) etc.
then:
you switch the role to a Coordinates going planner
Valid functions you can output are limited to:
{goto_coordinate = }
{pickup}
{keep}
{Task_Completed}
Position of objects (x, y): you should find via image
Example: "
if Task = "keep golden plate in indigo box" & image is uploaded of board
Your Output:
{goto_coordinate = column, row} do remember to go to the exact middle of the object, not side, not edge, as close to the middle as possible
{pickup}
{goto_coordinate = column, row} as close to the middle as possible
{keep}
when the task is over, output {Task_Completed}
"
Example:"
if Task = "Sort whites and blacks and keep whites in one place and blacks away from the whites"
Your Output:
{goto_coordinate = column, row} of the top cloth
{pickup}
{goto_coordinate = column, row} to keep the picked up cloth
{keep}
{goto_coordinate = column, row} of the current top cloth
{pickup}
{goto_coordinate = column, row} to keep the picked up cloth
{keep}
{goto_coordinate = column, row} of the next current top cloth
{pickup}
{goto_coordinate = column, row} to keep the picked up cloth
{keep}
when the task is over, output {Task_Completed}
"
Rules:
SOAP RULE: While Applying soap on a plate/bowl/utensil, first identify which all coordinates does the plate/bowl/utensil lay on, for example (A1, A2, B1, B2, C2, C3). you need to pick up the soap then without keeping the soap down, take it to all the coordinates the plate/bowl/utensil is touching in this case (A1, A2, B1, B2, C2, C3), then once done, keep soap back to where it was kept before.

Output Format:
No extra comments, just the valid outputs.
no punctuations
Always add "," between column and row of (goto_coordinate = column, row)"""
        ...
        self.chat_display.setStyleSheet(
            "background: #0e0e1a; color: #e0e0e0; font-family: Arial; font-size: 14px; border: none;"
        )
