...
REL_FALLBACK    = 0.45
MAX_TOUCH_CELLS = 24          # cap for regular (non-surface) objects
# Surfaces (tables, counters, sinks — vision reports surface:true) are exempt
# from the cap: the robot must sweep/wipe EVERY cell they cover. The old
# uniform 24-cell cap silently clipped large tables to their left columns.
...
- ACCURACY CHECK for small objects (soap, cups, tools, phones): before
  finalizing, verify the polygon sits ON the object's visible pixels — its
  centroid must land inside the object, not on adjacent floor or background.
  A polygon shifted even slightly off a small object makes the robot grab
  empty space. Tight and centered beats loose and roomy; never pad a small
  object's polygon with surrounding floor.
- For appliances and furniture, outline the FULL visible body of the object
  from this camera's viewpoint. For openable appliances (washing machine,
  oven, bin), the polygon MUST cover the opening — the drum mouth, door, or
  lid — because the robot loads/unloads items at the polygon's CENTER. Never
  outline the patch of floor beside or below the appliance.
...
# The planner only ever runs on non-dexterous tasks.
# ─────────────────────────────────────────────────────────────────────────────
DEXTERITY_SYSTEM = (
    """
...
# ─────────────────────────────────────────────────────────────────────────────
def _trim_uniform(touches: list, limit: int) -> list:
    """Spatially uniform downsample: keep every k-th cell of the (row,col)-sorted
    list instead of clipping one side of the object. Preserves overall extent."""
    if len(touches) <= limit:
        return touches
    ordered = sorted(touches, key=lambda t: (t[1], t[0]))
    step = len(ordered) / float(limit)
    return [ordered[int(i * step)] for i in range(limit)]


def bbox_to_cells(box, thr=TOUCH_THRESHOLD, is_surface=False):
    ...
    Surfaces (is_surface=True) are never capped — every covered cell is kept so
    sweep/wipe commands span the whole surface.
    """
    try:
    ...
    if not is_surface and len(touches) > MAX_TOUCH_CELLS:
        # Keep the best-covered cells (contiguous core), dropping the thin
        # boundary slivers first. Surfaces are exempt from the cap entirely,
        # so the old "left-columns-only" clipping bug cannot recur here.
...
def polygon_to_cells(polygon, thr=TOUCH_THRESHOLD, is_surface=False):
    ...
    thin object only claims the cells it actually occupies. Surfaces are
    never capped — the full covered area is kept."""
    try:
    ...
    if len(touches) > MAX_TOUCH_CELLS and not is_surface:
        # Coverage-ranked keep: contiguous core, boundary slivers dropped.
        ranked  = sorted(scored, key=lambda s: -s[0])
        keep    = {c for _, c in ranked[:MAX_TOUCH_CELLS]}
        touches = [c for c in touches if c in keep]
    ...
    def run(self):
        try:
            ...
            for obj in data.get("objects", []):
                is_surface = bool(obj.get("surface", False))
                polygon = obj.get("polygon")
                if isinstance(polygon, list) and len(polygon) >= 3:
                    center, touches = polygon_to_cells(polygon, is_surface=is_surface)
                    box = _poly_bbox(polygon)
                else:
                    # Fallback for older-style bbox-only responses.
                    box = obj.get("box")
                    center, touches = bbox_to_cells(box, is_surface=is_surface)
                if center is None:
                    continue
                objs.append({
...
class DexterityWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Strict-then-lenient parse; fail-safe defaults to dexterous
            # (the classifier's own rule: when in doubt, dexterous).
...
class AISidebar(QWidget):
    ...
    def _build_ui(self):
        ...
        sub = QLabel("Auto Vision  +  Dexterity Gate")
        ...
        # ── STEP 1 · Task (vision now runs automatically on image import) ────
    ...
    def auto_analyse(self):
        ...
        self._lock(True)
        self._play_btn.setEnabled(False)
        self._set_stage("🔍  Analysing imported image…")
        self._cmd_box.clear()
    ...
    def feed_frame(self, bgr):
        if bgr is None:
            ...
            return
        self._set_stage("🔍  Detecting objects…")
    ...
    def _on_run(self):
        ...
        if not self._all_objs:
            self._set_stage("⚠️  Import an image first so vision can detect objects", C_RED); return
        self._lock(True)
        self._play_btn.setEnabled(False)
        self._cmd_box.clear()
        self._set_stage("⚡  Checking task feasibility…")
        if self._instructions:
            notes = "\n".join(f"- {s}" for s in self._instructions)
            task  = f"{task}\n\nADDITIONAL AI INSTRUCTIONS (apply throughout):\n{notes}"
        # Dexterity gate first — only the raw user task is classified,
        # never the OBJECT LIST or extra instructions.
        self._pending_task = task
        self._dexterity_worker = DexterityWorker(self._task_input.toPlainText().strip())
        self._dexterity_worker.verdict.connect(self._on_dexterity_verdict)
        self._dexterity_worker.error.connect(self._on_error)
        self._dexterity_worker.start()
    ...
    def _on_dexterity_verdict(self, verdict: str):
        if verdict == "dexterous":
            ...
            return
        # Non-dexterous → proceed straight to the planner, no extra output.
...
class CameraPanel(QWidget):
    ...
    def _import_image(self):
        ...
        # Auto-run vision analysis on every successful import.
