...
PROMPT_HISTORY_PATH      = os.path.join(HOS_DATA_DIR, "prmp_hstry.json")
...
def encode_jpeg_b64(bgr, quality=96):
...
def component_on_parent(poly, parent_poly) -> bool:
    """Is this component outline actually ON the object it belongs to?

    Parts are found in a separate close-up pass, on a CROP of the object, and
    their coordinates are mapped back to the full frame through the crop rect.
    Nothing downstream re-checks that result against the parent, so a part the
    close-up model placed badly published a perfectly-formatted `handle@C9`
    sitting nowhere near the object — and since a part cell BEATS the object's
    CENTER when the gripper picks up, the arm drove to that cell and closed on
    empty board while the outline on screen still looked right.

    A part is accepted when its centroid lies inside the parent outline, or
    when at least half of it does. Anything less is the crop mapping having
    gone wrong, and the part keeps its name but loses its geometry.
    """
    if not isinstance(parent_poly, (list, tuple)) or len(parent_poly) < 3:
        return True
    # A malformed parent outline must not decide anything. Every other polygon
    # consumer in this file coerces inside a try; this one has to as well, or a
    # single bad vertex takes down the whole parts pass. Failing open is the
    # safe direction: the grip-cell gate downstream works on cell labels, not
    # polygons, so the gripper is still protected.
    try:
        parent = [(float(x), float(y)) for x, y in parent_poly]
    except (TypeError, ValueError):
        print("[parts] parent outline is malformed — on-parent check skipped")
        return True
    try:
    ...
    if not total:
        return False
    return inside / float(total) >= COMPONENT_ON_PARENT_MIN


def localise_component(comp, parent=None):
    """Assign center cell / touches / rounded polygon on one component dict.

    `parent` is the object dict the component belongs to; when given, a part
    outline that does not actually land on the parent is rejected outright
    (see component_on_parent) rather than published as a cell the gripper
    would then aim at.
    """
    poly = comp.get('polygon')
    if not isinstance(poly, (list, tuple)) or len(poly) < 3:
        return False
    if parent is not None and not component_on_parent(poly, parent.get('polygon')):
        print(f"[parts] {comp.get('name', 'part')}: outline is off "
              f"'{parent.get('name', 'object')}' — geometry dropped")
        return False
...
## TASKS THAT POINT BACKWARDS

"Do it again", "same as before", "repeat that", "continue", "carry on" - a task like this names no object and no action, so on its own it is unactionable. But it is usually NOT a question worth asking, because you are given a RECENT TASKS list and the answer is normally sitting on its last line. If exactly one earlier task is plausibly being referred to, the task is CLEAR: the planner is given the same list and will resolve it the same way. Do not ask "did you mean the last one?" when there is only one thing it can mean.

Ask only when the back-reference is genuinely ambiguous - several recent tasks it could equally mean - and then make each option the actual wording of one of those earlier tasks, so the operator is choosing between real tasks rather than restating "the same as before". Never write an option that points backwards again ("the same thing as before") - that is the very phrase that needs resolving, and offering it back resolves nothing.

If there is no RECENT TASKS list at all, or it is empty, then there is nothing to refer to: ask what the operator wants done, with options built from the OBJECT LIST.
...
## WHEN THE TASK POINTS BACKWARDS

"Do it again", "same as before", "repeat that", "carry on with that one" - these are not tasks, and handing one to the planner as if it were is a guaranteed failure. When the original task or the operator's ANSWER points back at earlier work, you will have been given a RECENT TASKS list. Find the task being pointed at - normally the most recent line - and OUTPUT THAT TASK'S WORDING as the instruction. Not "do the same thing as before". Not "repeat the last task". The actual words of the actual task.

Original: "do it again"
Q: "Do the same thing as before, or something else?"  A: "Do the same thing as before"
RECENT TASKS ends with: "wash the clothes in the basin"
Output: Wash the clothes in the basin.

If the answer instead points at something else ("something else", "no, the jeans"), the back-reference is dropped and their answer is the task, as usual. If there is no RECENT TASKS list and nothing to point at, say what the operator asked for as plainly as you can and let the planner report it.
...
class VisionWorker(QThread):
    ...
    def _localise(self, objs):
        ...
        for o in live:
            ...
            for c in comps:
                c.pop('_sq', None)
                if not localise_component(c, parent=o):
                    c.pop('polygon', None)
                    c.pop('box', None)
                    c.pop('center', None)
                    c.pop('touches', None)
    ...
    def _detect_parts(self, client, obj):
        ...
        for c in comps:
            c.pop('_sq', None)
            if not localise_component(c, parent=obj):
...
def cell_on_object(o, cell: str, slack: int = GRIP_CELL_SLACK) -> bool:
    ...
    The grip cell is what the planner drives to instead of CENTER, so a cell
    that is not on the object is not a worse grasp - it is a move to empty
    board. Everything handed to the planner is checked against the object's
    own TOUCHES here. The slack ring keeps a legitimate grasp feature that
    juts a cell past a tight outline (a handle, a spout) while still ruling
    out anything genuinely elsewhere in the scene.
    """
    want = parse_cell(str(cell or ''))
    if want is None:
        return False
    known = [parse_cell(c) for c in object_cells(o)]
...
def default_grip_part(o):
    ...
    for c in parse_component_entries(o.get('components')):
        ...
        if not cell_on_object(o, cell):
            print(f"[grip] {o.get('name', 'object')}: part '{name}' cell {cell} "
                  f"is not on the object — ignored")
            continue
...
def resolve_grip_cells(grips: list, objs: list) -> list:
    ...
    for g in grips or []:
        ...
        if cell and not cell_on_object(o, cell):
            # The named part exists but its measured cell is off the object -
            # a part pass that mapped back badly. Fall through to the rest of
            # the ladder rather than driving the gripper to open board.
            print(f"[grip] {name}: part '{part}' cell {cell} is off the "
                  f"object — falling back")
            part, cell = None, None
        if not cell:
...
class ClarityWorker(QThread):
    ...
    def run(self):
        try:
            client = make_client()
            parts = [f"OBJECT LIST:\n{self._objs}"]
            past = prompt_history_block(self._task)
            if past:
                parts.append(past)
            parts.append(f"TASK:\n{self._task}")
            user = "\n\n".join(parts)
...
class CommandWorker(QThread):
    ...
    def run(self):
        try:
            client = make_client()
            parts = [f"OBJECT LIST:\n{self._objects}"]
            past = prompt_history_block(self._task, with_plan=True)
            if past:
                parts.append(past)
...
# ---------------------------------------------------------------------------
# Prompt history — every task the operator has asked for, and what became of it
#
# Written to prmp_hstry.json in HOS data so a task survives the app closing.
# The point is not an audit log: it is being able to pick a task back up. A run
# that was interrupted, a task that was phrased exactly right the first time, a
# plan worth sending again - all of them are one click away instead of being
# retyped from memory.
#
# Like every other store in HOS data this fails open in both directions: a
# missing or corrupt file reads as empty history, and a write that fails is
# swallowed. History is a convenience, and a convenience must never be able to
# stop a run.
# ---------------------------------------------------------------------------

PROMPT_HISTORY_MAX = 400


def load_prompt_history() -> list:
    """Every task ever submitted, oldest first."""
    try:
        with open(PROMPT_HISTORY_PATH, encoding="utf-8") as f:
            entries = json.load(f)
...
def _write_prompt_history(entries: list) -> None:
    try:
        os.makedirs(HOS_DATA_DIR, exist_ok=True)
        tmp = PROMPT_HISTORY_PATH + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(entries[-PROMPT_HISTORY_MAX:], f, indent=2, ensure_ascii=False)
        os.replace(tmp, PROMPT_HISTORY_PATH)
    except Exception:
        pass


def append_prompt_history(task: str, **fields) -> dict:
    """Record a task the moment it is submitted, before anything can fail.

    Returns the stored record - the caller keeps its `id` so the same entry can
    be updated as the run progresses, rather than writing a second entry for
    every stage.
    """
    ...
    stored = {
        "id":        max((int(e.get("id") or 0) for e in entries), default=0) + 1,
    ...
    _write_prompt_history(entries)
    return stored


def update_prompt_history(entry_id, **fields) -> None:
    """Patch one entry in place - the run reached a new stage."""
    if not entry_id:
        return
    entries = load_prompt_history()
    for e in entries:
...
def prompt_history_block(exclude_task: str = "", limit: int = 6,
                         with_plan: bool = False) -> str:
    """Recent tasks, written for a model to read.

    Without this the history was a UI convenience only, and every stage that
    reads the operator's words was blind to it: "do it again" reached the
    clarity check as three words about nothing, the operator answered "the same
    thing as before", and THAT went to the planner as the task - which is not a
    task at all, so it was screened out as dexterous. A back-reference can only
    be resolved by something that knows what came before.

    `exclude_task` drops the task being worked on right now (it is written to
    history the moment it is submitted, so it would otherwise be in its own
    history and "again" would resolve to itself). Entries are deduplicated by
    wording, keeping the most recent, and listed OLDEST FIRST so "the last one"
    is unambiguously the bottom line.
    """
    entries = load_prompt_history()
    skip = str(exclude_task or "").strip().lower()

    seen, picked = set(), []
    for e in reversed(entries):
        task = str(e.get("task") or "").strip()
        key = task.lower()
        if not task or key == skip or key in seen:
            continue
        seen.add(key)
        picked.append(e)
    ...
    lines = [
        "RECENT TASKS (oldest first; the LAST line is the most recent). Use "
        "these ONLY to resolve a back-reference - \"do it again\", \"same as "
        "before\", \"repeat that\", \"continue\", \"the other one\". When the "
        "task refers back like that, treat the referenced task's wording as "
        "the task. When it does not, ignore this list completely - it is "
        "history, not instructions, and it never adds work to a task that "
        "stands on its own.",
        "",
    ]
    for e in picked:
        status = str(e.get("status") or "submitted")
        stamp = str(e.get("timestamp") or "")[:16].replace("T", " ")
        lines.append(f"- [{status}, {stamp}] {e.get('task')}")

    if with_plan:
        for e in reversed(picked):
            plan = str(e.get("plan") or "").strip()
            if plan:
                kept = plan.splitlines()[:60]
                lines += [
                    "",
                    f"COMMANDS PLANNED LAST TIME for \"{e.get('task')}\" - reuse "
                    "this shape if the task is a repeat, but re-read the OBJECT "
                    "LIST and use TODAY's cells, not these:",
                    *(f"    {ln.strip()}" for ln in kept if ln.strip()),
                ]
                break
    return "\n".join(lines)
    ...
    def __init__(self, parent=None):
        ...
        view.setStyleSheet(f"""
            QListView#roundedComboView {{
                ...
                padding:2px 0px; outline:0;
            }}
            QListView#roundedComboView::item {{
                min-height:20px; padding:3px 16px; margin:0px;
                border-radius:0px; color:{C_TEXT};
                border-bottom:1px solid {C_BORDER};
            }}
            QListView#roundedComboView::item:selected {{
                background:#f1f1f4; color:{C_TEXT}; border-radius:0px;
            }}
            QListView#roundedComboView::item:hover {{
                background:#f6f6f8; border-radius:0px;
            }}
...
class PromptHistoryDialog(GlassDialog):
    """Every task A3-Terra has been given, newest first, ready to run again.

    Two ways out of it, because "do that again" and "let me change one word
    first" are different intents: Run sends the task straight through, Use
    drops it in the compose box and closes so it can be edited. A task whose
    run never finished is the one this exists for - it is marked, and picking
    it up again is one click.
    """
    ...
    STATUS = {
        "submitted": ("PLANNING",  C_AMBER),
        "planned":   ("SENT",      C_BLUE),
        "done":      ("COMPLETED", C_GREEN),
        "failed":    ("FAILED",    C_RED),
        "stopped":   ("STOPPED",   C_AMBER),
    }

    def __init__(self, parent=None):
        super().__init__(
            "Prompt history", parent,
            subtitle="Every task you have asked for, newest first. Run one "
                     "again as it was, or drop it back in the box to edit.",
            width=620)
        self.resize(620, 620)
        self._scroll = VScrollArea()
        ...
        row = QHBoxLayout(); row.setSpacing(9)
        self._clear = pill_button("Clear history", height=32)
        self._clear.clicked.connect(self._on_clear)
        close = pill_button("Close", primary=True, height=32)
        close.clicked.connect(self.accept)
        row.addWidget(self._clear, 0); row.addStretch(1); row.addWidget(close, 0)
        self.body.addLayout(row)
    ...
    def _rebuild(self):
        ...
        entries = list(reversed(load_prompt_history()))
        self._clear.setEnabled(bool(entries))
        if not entries:
            empty = QLabel("Nothing yet — the tasks you run will collect here.")
            empty.setWordWrap(True)
            empty.setFont(QFont(UI_FONT, 10))
            empty.setStyleSheet(f"color:{C_TEXT_DIM};background:transparent;"
                                f"border:none;padding:18px 4px;")
            self._col.addWidget(empty)
            self._col.addStretch(1)
            return

        for e in entries:
            self._col.addWidget(self._card(e))
        self._col.addStretch(1)

    def _card(self, e: dict) -> QWidget:
        task = str(e.get("task") or "").strip()
        label, colour = self.STATUS.get(str(e.get("status") or ""),
                                        ("SUBMITTED", C_TEXT_DIM))

        card = QFrame()
        card.setStyleSheet(
            f"QFrame{{background:rgba(255,255,255,0.72);"
            f"border:1px solid {C_BORDER};border-radius:18px;}}")
        lay = QVBoxLayout(card)
        lay.setContentsMargins(14, 11, 14, 11)
        lay.setSpacing(7)

        head = QLabel(f"{label}  ·  {self._when(e.get('timestamp'))}")
        head.setFont(QFont(UI_FONT, 8, QFont.Bold))
        head.setStyleSheet(f"color:{colour};background:transparent;border:none;"
                           f"letter-spacing:0.06em;")
        lay.addWidget(head)

        body = QLabel(task)
        body.setWordWrap(True)
        body.setFont(QFont(UI_FONT, 10))
        body.setStyleSheet(f"color:{C_TEXT};background:transparent;border:none;")
        lay.addWidget(body)

        row = QHBoxLayout(); row.setSpacing(8)
        row.addStretch(1)
        use = pill_button("Use", height=28)
        use.clicked.connect(lambda _=False, t=task: (self.use.emit(t), self.accept()))
        again = pill_button("Run again", primary=True, height=28)
        again.clicked.connect(lambda _=False, t=task: (self.run.emit(t), self.accept()))
        row.addWidget(use, 0); row.addWidget(again, 0)
        lay.addLayout(row)
        return card

    @staticmethod
    def _when(stamp) -> str:
        """'today 14:32' reads faster than a full ISO timestamp."""
        try:
...
class AISidebar(QWidget):
    ...
    def _build_chat_ui(self):
        ...
        self._history_btn.setFont(QFont(UI_FONT, 13, QFont.Bold))
        self._history_btn.setToolTip("Prompt history \u00b7 run an earlier task again")
        self._history_btn.setStyleSheet(
            f"QPushButton{{background:rgba(139,92,246,0.10);color:{C_TEXT_DIM};"
            f"border:1.5px solid {C_BORDER};border-radius:18px;}}"
            f"QPushButton:hover{{color:{C_VIOLET};border-color:#c4b5fd;}}")
        self._history_btn.clicked.connect(self._open_prompt_history)
        ...
        cl.addWidget(self._history_btn, 0, Qt.AlignBottom)
    ...
    def _open_prompt_history(self):
        """Past tasks, with the option to run one again."""
        dlg = PromptHistoryDialog(self)
        dlg.use.connect(self._recall_task)
        dlg.run.connect(self._rerun_task)
        dlg.exec()

    def _recall_task(self, task: str):
        """Put the old wording back in the box, unsent, ready to edit."""
        if not task:
            return
        self._task_input.setPlainText(task)
        self._task_input.setFocus()
        self._set_stage("Recalled from history \u2014 edit it, then send.")

    def _rerun_task(self, task: str):
        """Continue an earlier task exactly as it was written.

        Goes through _on_run like any other submission, so it re-uses the
        current board and the current vision - continuing a task means doing
        it to the board as it is NOW, not as it was when it was first asked.
        """
        if not task:
            return
        if self._busy():
            self._set_stage("\u23f3  Still working on the last task", C_AMBER)
            return
        if self._last_frame is None:
            self._set_stage("Import an image first, then run it again", C_RED)
            self._task_input.setPlainText(task)
            return
        self._task_input.setPlainText(task)
        self._on_run()
    ...
    def _chat_error(self, err, summary="Not able to complete task."):
        ...
        """
        update_prompt_history(getattr(self, "_history_id", None), status="failed")
        self._end_thinking()
    ...
    def _uploaded_view_kinds(self) -> list:
        """The real angles the operator has supplied (excludes 'original',
        which is just a copy of whichever angle landed first)."""
        return [k for k in VIEW_KINDS if k in self._views_by_kind]

    def _maybe_autorun_vision(self) -> bool:
        """With a single view there is nothing for the chooser to pick
        between, so analysis can start the moment that view is uploaded -
        no need to wait for a task. Returns True if vision was started."""
        if len(self._uploaded_view_kinds()) != 1:
            return False
        if self._last_frame is None or self._vision_objs:
            return False
        if self._busy() or self._vision_busy():
            return False
        if self._task_input.toPlainText().strip():
            return False
        self._lock(True)
        self._run_view_chooser(None)
        return True
    ...
    def _on_run(self):
        ...
        # Written before anything can fail, so a task that crashes the run is
        # still in the history to be picked up again.
        self._history_id = (append_prompt_history(task) or {}).get("id")
        self._err_task = task
        self._approved_flow = []
    ...
    def _on_cmd_done(self, _full: str):
        ...
        bubble.enable_copy()
        update_prompt_history(getattr(self, "_history_id", None),
                              status="planned", plan=self._cmd_text.strip())
    ...
    def _on_stop(self):
        update_prompt_history(getattr(self, "_history_id", None), status="stopped")
        self._cancel_everything()

    def on_runner_finished(self):
        update_prompt_history(getattr(self, "_history_id", None), status="done")
        self._exec_busy = False
        self._stop_btn.setEnabled(False)
        self._inline_stop_btn.setEnabled(False)
        self._refresh_run_btn()
...
PLUGINS_DIR = os.path.join(HOS_DATA_DIR, "plugins")
PLUGINS_ENABLED_PATH = os.path.join(PLUGINS_DIR, "enabled.json")


class PluginManager:
    """Loads .py files from HOS/plugins/. Each plugin defines register(main_window)."""
    ...
    def _enabled_set(self) -> set:
        try:
            with open(PLUGINS_ENABLED_PATH, "r", encoding="utf-8") as fh:
                return set(json.load(fh))
        except (OSError, ValueError):
            return set()

    def _save_enabled(self, names: set):
        with open(PLUGINS_ENABLED_PATH, "w", encoding="utf-8") as fh:
            json.dump(sorted(names), fh, indent=2)

    def available_plugins(self):
        """Filenames (without .py) of every plugin in the folder."""
    ...
    def load_enabled_on_startup(self):
        for name in self._enabled_set():
            self.enable(name, quiet=True)

    def enable(self, name: str, quiet: bool = False) -> bool:
        """Import the plugin and call register(). Returns True on success."""
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(f"a3terra_plugin_{name}", path)
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            register_fn = getattr(mod, "register", None)
            if register_fn is not None:
                register_fn(self._win)
            self._loaded[name] = mod
        except Exception:
            if not quiet:
                QMessageBox.warning(
                    self._win, "Plugin failed",
    ...
    def disable(self, name: str):
        """Marks the plugin as not-auto-loaded. Does not unload a live plugin
        (Python has no clean way to undo arbitrary register() side effects) —
    ...
    def plugin_info(self, name: str):
        """(display name, description) without importing the module fully."""
        path = os.path.join(PLUGINS_DIR, f"{name}.py")
        display, desc = name, ""
        try:
            with open(path, "r", encoding="utf-8") as fh:
                src = fh.read()
            m = re.search(r'^PLUGIN_NAME\s*=\s*["\'](.+?)["\']', src, re.M)
            if m:
                display = m.group(1)
            m = re.search(r'^PLUGIN_DESC\s*=\s*["\'](.+?)["\']', src, re.M)
            if m:
                desc = m.group(1)
        except OSError:
            pass
    ...
    def import_plugin_file(self, src_path: str) -> str:
        """Copies an external .py into the plugins folder. Returns its name."""
        base = os.path.splitext(os.path.basename(src_path))[0]
        dest = os.path.join(PLUGINS_DIR, f"{base}.py")
        shutil.copyfile(src_path, dest)
        return base


class PluginRow(QFrame):
    """One plugin: name + description on the left, an iOS-style switch on
    the right — same row language as InstructionRow/Settings, so the
    Plugins popup reads as part of the app rather than a bolted-on dialog."""

    toggled = Signal(bool)

    def __init__(self, name: str, display: str, desc: str, checked: bool, parent=None):
        super().__init__(parent)
        self.name = name
        self.setStyleSheet(
            f"PluginRow{{background:rgba(255,255,255,0.72);"
            f"border:1px solid {C_BORDER};border-radius:18px;}}"
            f"PluginRow:hover{{background:rgba(255,255,255,0.88);}}")
        h = QHBoxLayout(self)
        ...
        if desc:
            ...
            sub.setWordWrap(True)
            sub.setFont(QFont(UI_FONT, 9))
            sub.setStyleSheet(f"color:{C_TEXT_DIM};background:transparent;border:none;")
            text_col.addWidget(sub)
        h.addLayout(text_col, 1)

        self._switch = ToggleSwitch(checked)
        self._switch.toggled.connect(self.toggled)
        h.addWidget(self._switch, 0, Qt.AlignVCenter)
...
class PluginsDialog(GlassDialog):
    """Plugins ▸ Browse Plugins… — toggleable card list + import, styled
    like every other pop-up in the app (frameless glass sheet)."""

    def __init__(self, manager: PluginManager, parent=None):
        super().__init__("Plugins", parent,
                          subtitle="Enable or disable plugins. A newly-enabled "
        ...
        self._rows_lay = QVBoxLayout(holder)
        ...
        footer = QHBoxLayout()
        import_btn = QPushButton("Import Plugin (.py)…")
        import_btn.setCursor(Qt.PointingHandCursor)
        import_btn.setStyleSheet(
            f"QPushButton{{background:{C_BTN};color:{C_BTN_FG};border:none;"
            f"border-radius:16px;padding:8px 16px;font-size:10px;font-weight:600;}}"
            f"QPushButton:hover{{background:{C_BTN_HOVER};}}")
    ...
    def _populate(self):
        ...
        names = self._mgr.available_plugins()
        if not names:
            empty = QLabel("No plugins yet — import a .py file to get started.")
            empty.setFont(QFont(UI_FONT, 10))
            empty.setStyleSheet(f"color:{C_TEXT_DIM};background:transparent;")
        ...
        for name in names:
            display, desc = self._mgr.plugin_info(name)
            row = PluginRow(name, display, desc, self._mgr.is_enabled(name))
            row.toggled.connect(
                lambda on, r=row: self._on_toggled(r, on))
            self._rows_lay.addWidget(row)
            self._rows.append(row)
        self._rows_lay.addStretch(1)

    def _on_toggled(self, row: "PluginRow", on: bool):
        if on:
            if not self._mgr.enable(row.name):
                row.setChecked(False)
        else:
            self._mgr.disable(row.name)

    def _on_import(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Import Plugin", "", "Python files (*.py)")
        if not path:
            return
        try:
            self._mgr.import_plugin_file(path)
        except OSError as e:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._build_menus()
        self._plugins.load_enabled_on_startup()
    ...
    def _build_menus(self):
        ...
        self._plugins_menu = plugins_menu = QMenu("Plugins", self)
        bar.addMenu(plugins_menu)

        act_browse_plugins = QAction("Browse Plugins…", self)
        act_browse_plugins.setStatusTip("Enable, disable, or import plugins")
        act_browse_plugins.triggered.connect(self._open_plugins_dialog)
        plugins_menu.addAction(act_browse_plugins)

        plugins_menu.addSeparator()

        act_import_plugin = QAction("Import Plugin (.py)…", self)
        act_import_plugin.triggered.connect(self._import_plugin)
        plugins_menu.addAction(act_import_plugin)
    ...
    def _open_plugins_dialog(self):
        """Plugins ▸ Browse Plugins… — enable/disable/import."""
        PluginsDialog(self._plugins, self).exec()

    def _import_plugin(self):
        """Plugins ▸ Import Plugin (.py)… — copy in, then open Browse to enable."""
        path, _ = QFileDialog.getOpenFileName(
            self, "Import Plugin", "", "Python files (*.py)")
        if not path:
            return
        try:
            self._plugins.import_plugin_file(path)
        except OSError as e:
            QMessageBox.warning(self, "Import failed", str(e))
            return
        self._open_plugins_dialog()
