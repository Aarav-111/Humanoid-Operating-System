...
<head>
    ...
    <style>
        ...
        .hos-toggle-wrap:hover { border-color: #6366f1; }
        .hos-toggle-wrap.active { border-color: #6366f1; background: rgba(99,102,241,0.1); }
        .hos-switch {
            width: 32px;
            height: 18px;
            background: #3f3f46;
        ...
        .hos-switch.on { background: #6366f1; }
        .hos-switch::after {
            content: '';
            position: absolute;
            width: 12px;
        ...
        .hos-switch.on::after { left: 17px; }
        .hos-label { font-size: 11px; font-weight: 700; color: #71717a; letter-spacing: 0.06em; transition: color 0.2s; }
        .hos-toggle-wrap.active .hos-label { color: #a5b4fc; }

        /* HOS stage indicator in chat */
        .hos-stage-badge {
        ...
        .hos-badge-planning { background: rgba(99,102,241,0.18); color: #a5b4fc; border: 1px solid #6366f1; }
        .hos-badge-executing { background: rgba(251,191,36,0.12); color: #fbbf24; border: 1px solid #d97706; }
    </style>
</head>
<body class="bg-zinc-950 text-zinc-200 overflow-hidden">
    <div class="ui-overlay top-0 left-0 right-0 bg-black/90 backdrop-blur border-b border-zinc-800 px-6 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-x-3">
            <!-- HOS AI Toggle -->
            <div id="hos-toggle" class="hos-toggle-wrap" onclick="toggleHOS()">
                <div id="hos-switch" class="hos-switch"></div>
                <span class="hos-label" id="hos-label">HOS AI</span>
            </div>
        ...
        <div class="bg-zinc-900/95 backdrop-blur border border-zinc-700/60 rounded-2xl flex flex-col flex-1 min-h-0">
            <div class="flex items-center gap-2 px-4 pt-3 pb-3 border-b border-zinc-700/60 shrink-0">
                ...
                <div>
                    ...
                    <div class="text-xs text-zinc-500 mono" id="planner-subtitle">Prolabs V12.2 · Claude</div>
                </div>
            ...
            <div id="chat-messages" class="flex-1 overflow-y-auto px-3 py-3 space-y-2 min-h-0">
                <div class="flex gap-2">
                    ...
                    <div class="bg-zinc-800 rounded-xl rounded-tl-sm px-3 py-2 text-xs text-zinc-300 leading-relaxed">Hi! I'm the K3D Task Planner. Tell me what to do — I'll plan and execute the moves automatically. Try: <span class="text-blue-400">"Move bottle to C3"</span>, <span class="text-blue-400">"Pick up the box and place it at F5"</span>, or <span class="text-blue-400">"Sort all objects by type"</span>.<br><br><span class="text-indigo-400 text-xs">Enable <b>HOS AI</b> in the header for high-level task understanding.</span></div>
                </div>
            </div>
    ...
    <script>
        ...
        // HOS AI state
        let hosEnabled = false;

        window.toggleHOS = function() {
            hosEnabled = !hosEnabled;
            const sw = document.getElementById('hos-switch');
            const wrap = document.getElementById('hos-toggle');
            const subtitle = document.getElementById('planner-subtitle');
            if (hosEnabled) {
                sw.classList.add('on');
                wrap.classList.add('active');
                subtitle.textContent = 'Prolabs V12.2 · HOS AI → K3D';
                appendMessage('assistant',
                    '<span class="hos-stage-badge hos-badge-planning">HOS AI ON</span><br>' +
                    'High-level planner enabled. I\'ll now decompose your instructions into subtasks before executing.');
            } else {
                sw.classList.remove('on');
                wrap.classList.remove('active');
                subtitle.textContent = 'Prolabs V12.2 · Claude';
                appendMessage('assistant',
                    '<span class="hos-stage-badge" style="background:rgba(113,113,122,0.2);color:#71717a;border:1px solid #52525b;">HOS AI OFF</span><br>' +
                    'Direct execution mode. Commands go straight to the motion planner.');
            }
        };
        ...
        function appendThinking(label) {
            ...
            wrap.innerHTML = '<div class="w-5 h-5 bg-blue-600 rounded-md flex items-center justify-center text-white text-xs font-bold shrink-0 mt-0.5">AI</div>' +
                '<div class="bg-zinc-800 rounded-xl rounded-tl-sm px-3 py-2 text-xs text-zinc-500 italic">' + (label || 'Planning task...') + '</div>';
            box.appendChild(wrap);
            box.scrollTop = box.scrollHeight;
        }
        function updateThinking(label) {
            const b = document.getElementById('thinking-bubble');
            if (b) b.querySelector('div:last-child').textContent = label;
        }
        ...
        const HOS_SYSTEM_PROMPT = `You are "HOS AI", a high-level planner for a humanoid robot.
Your job is to convert a user's instruction into clear, ordered, step-by-step subtasks.
You will ALWAYS receive:
1. An instruction
2. A board state description
------------------------
CORE BEHAVIOR
------------------------
1. Board State First
- Always analyze the board state before generating steps
- Identify all relevant objects
- Understand their positions, categories, and relationships
- Use this understanding to guide the task breakdown
2. Output Format
...
- The final line MUST be:
  task completed
3. Action Style (STRICT)
- ONLY use the verb: "Keep"
- Do NOT use any other verbs such as:
  pick, grab, move, place, lift, carry, etc.
...
- Do NOT include:
  ...
  - Angles
  - Motion details
- Focus only on WHAT to do, not HOW to do it
5. Object Handling Logic
- Break tasks into individual items when meaningful
- Use ordering terms when multiple items exist:
  first, second, third, etc.
- Maintain logical sequencing
6. Simple Task Rule (CRITICAL)
- If the task is naturally atomic and does NOT meaningfully benefit from subdivision:
  - Output ONLY ONE step
  - That step MUST be the EXACT original user instruction
  - Do NOT rephrase, summarize, or modify it in any way
- This rule overrides the "Keep" verb constraint for such cases
7. Interaction Logic
- When subdivision is needed, structure actions as:
  - Bring object to target
  - Perform action
  - Restore object if needed
8. SOAP RULE (STRICT)
- Applying soap is ALWAYS a SINGLE atomic step
- Do NOT break it into multiple steps
- Do NOT mention coordinates
- Do NOT describe coverage or motion
...
- No emojis`;
        ...
        const K3D_SYSTEM_PROMPT = `You are K3D — an autonomous task planner for the Prolabs V12.2 Precision Cartesian Gantry robot.
...
{Task_Completed}`;
        ...
        async function runHOSPipeline(userText, boardState) {
            updateThinking('HOS AI decomposing task...');

            const hosRes = await fetch('/api/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    system: HOS_SYSTEM_PROMPT,
                    messages: [{ role: 'user', content: `Instruction: ${userText}\n\nBoard state: ${boardState}` }]
                })
            });
            const hosData = await hosRes.json();
            const hosOutput = hosData.reply || '';

            const steps = hosOutput.split('\n')
                .map(l => l.trim())
                .filter(l => l && l !== 'task completed');

            appendMessage('assistant',
                '<span class="hos-stage-badge hos-badge-planning">HOS AI · SUBTASKS</span><br>' +
                '<pre class="text-xs mt-1 text-indigo-300 whitespace-pre-wrap">' + hosOutput + '</pre>');

            updateThinking('K3D planning motion commands...');

            const combinedTask = `The high-level planner decomposed the task into these subtasks:\n${hosOutput}\n\nBoard state: ${boardState}\n\nNow generate the full motion command sequence to execute all subtasks.`;

            const k3dRes = await fetch('/api/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    system: K3D_SYSTEM_PROMPT,
                    messages: [...chatHistory, { role: 'user', content: combinedTask }]
                })
            });
            const k3dData = await k3dRes.json();
            return k3dData.reply || '';
        }
        ...
        window.sendTask = async function() {
            ...
            try {
                ...
                if (hosEnabled) {
                    reply = await runHOSPipeline(text, boardState);
                    chatHistory.push({ role: 'user', content: `Task: ${text}\n\nCurrent board state: ${boardState}` });
                    ...
                    const res = await fetch('/api/chat', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ system: K3D_SYSTEM_PROMPT, messages: chatHistory })
                    });
                ...
                const badge = hosEnabled
                    ? '<span class="hos-stage-badge hos-badge-executing">K3D · EXECUTING</span><br>'
                    : '';
                const planText = commands.map((c, i) => `${i + 1}. ${c}`).join('\n');
                appendMessage('assistant', badge + `📋 Executing ${commands.length} steps:\n<pre class="text-xs mt-1 text-emerald-400">${planText}</pre>`);
...
OPENAI_API_KEY = "ADD YOU OPENAI API KEY HERE"
