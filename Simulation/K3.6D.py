    ...
    <script>
        ...
        function getBoardContext() {
            const objs = objects.map((o, i) => {
                ...
                return `${name} at ${String.fromCharCode(65 + x)}${z + 1}`;
            });
        ...
        function subTaskAction(message) {
            return new Promise(resolve => {
                setStatus(message);
        ...
        function parseMultiCoords(raw, keyword) {
            const coordStr = raw.replace(new RegExp(keyword, 'i'), '')
                                .replace(/[()=]/g, ' ')
                                .trim();
            return coordStr.split(',').map(s => s.trim()).filter(Boolean);
        }

        async function sortLaundry(raw) {
            const match = raw.match(/sort_laundry\((lights|darks)\s*,?\s*(.*)\)/i);
            const category = match ? match[1].charAt(0).toUpperCase() + match[1].slice(1).toLowerCase() : 'Lights';
            const items = match ? match[2].split(',').map(s => s.trim()).filter(Boolean) : [];
            setStatus(`Sorting laundry into ${category}...`);
            for (const item of items) {
                await subTaskAction(`Sorting ${item} into ${category}`);
            }
            setStatus(`✅ Sorted ${items.length} item(s) into ${category}`);
        }
        ...
        async function executeCommands(commands) {
            ...
            for (let i = 0; i < commands.length; i++) {
                ...
                } else if (rawLower === 'place_pod') {
                    await simpleAction('Placing coffee pod...');

                } else if (rawLower === 'press_button') {
                    await simpleAction('Pressing start button...');

                } else if (rawLower.startsWith('apply_sweep')) {
                    const coordTokens = parseMultiCoords(raw, 'apply_sweep');
                    setStatus(`🧹 Sweeping ${coordTokens.length} cell(s)...`);
                ...
                } else if (rawLower.startsWith('load_dishwasher')) {
                    const coordTokens = parseMultiCoords(raw, 'load_dishwasher');
                    setStatus(`🍽️ Loading ${coordTokens.length} dish(es)...`);
                    for (const token of coordTokens) {
                        const parsed = parseCoord(token);
                        if (parsed) await subTaskAction(`Placing dish at ${parsed.col}${parsed.row}`);
                    }
                    setStatus('✅ Dishwasher load complete');

                } else if (rawLower.startsWith('wipe_surface')) {
                    const coordTokens = parseMultiCoords(raw, 'wipe_surface');
                    setStatus(`🧽 Wiping ${coordTokens.length} area(s)...`);
                    for (const token of coordTokens) {
                        const parsed = parseCoord(token);
                        if (parsed) await subTaskAction(`Wiping surface at ${parsed.col}${parsed.row}`);
                    }
                    setStatus('✅ Surface wiped');

                } else if (rawLower.startsWith('sort_laundry')) {
                    await sortLaundry(raw);

                } else if (rawLower === 'take_out_trash') {
                    await simpleAction('Taking out trash bag...');

                } else if (rawLower === 'open_fridge') {
                    await simpleAction('Opening fridge...');

                } else if (rawLower === 'close_fridge') {
                    await simpleAction('Closing fridge...');

                } else if (rawLower === 'open_drawer') {
                    await simpleAction('Opening drawer...');

                } else if (rawLower === 'close_drawer') {
                    await simpleAction('Closing drawer...');

                } else if (rawLower === 'recover_dropped') {
                    await simpleAction('Recovering dropped object...');

                } else if (rawLower === 'set_table') {
                    await simpleAction('Setting the table...');

                } else if (rawLower === 'shelve_book') {
                    await simpleAction('Putting book on shelf...');

                } else if (rawLower === 'push_chair') {
                    await simpleAction('Pushing chair in...');
...
{Apply_sweep(X1, X2, X3, ...)}
{Load_dishwasher(X1, X2, X3, ...)}
{Wipe_surface(X1, X2, X3, ...)}
{Sort_laundry(Lights, X1, X2, ...)}
{Sort_laundry(Darks, X1, X2, ...)}
{Take_out_trash}
{Open_fridge}
{Close_fridge}
{Open_drawer}
...
COORDINATE FORMAT inside multi-cell commands: always ColRow with no space, e.g. A1, B3, T11.
...
TIDYING UP RULE:
1. Pick up large rigid items like books or boxes and place them on a shelf.
2. Use {Apply_sweep(...)} for small scattered items and move them into a dustpan or designated corner.
3. Do not mix sweeping and shelf placement in the same command sequence.
4. End with {Task_Completed}.

COFFEE MACHINE RULE:
1. Pick up the pod first with {pickup}.
2. Move to the machine slot and use {place_pod}.
3. Pick up the mug and position it under the nozzle with {goto_coordinate} and {keep}.
4. Use {press_button} to start the machine.
5. End with {Task_Completed}.

WATERING PLANTS RULE:
1. Pick up the watering can with {pickup}.
2. Move above the plant and pour gently with {pour}.
3. Avoid touching leaves and maintain a smooth pouring motion.
4. End with {Task_Completed}.

DISHWASHER RULE:
1. Pick up each plate from the rim with {pickup}.
2. Move to the rack and use {Load_dishwasher(...)} to place dishes into slots.
3. End with {Task_Completed}.

TABLE WIPE RULE:
1. Pick up the sponge or cloth with {pickup}.
2. Use {Wipe_surface(...)} and clean in a zig-zag pattern.
3. Keep downward pressure during the wipe.
4. End with {Task_Completed}.

SORT LAUNDRY RULE:
1. Pick up one clothing item with {pickup}.
2. Inspect the color and choose Lights or Darks.
3. Use {Sort_laundry(Lights, ...)} or {Sort_laundry(Darks, ...)}.
4. End with {Task_Completed}.

TRASH BAG RULE:
1. Pick up the exposed bag handle with {pickup}.
2. Pull straight up and move the bag to the collection spot.
3. Use {Take_out_trash} to complete the action.
4. End with {Task_Completed}.

FRIDGE DOOR RULE:
1. Use {Open_fridge} to open the door along a circular arc.
2. Wait for item transfer if needed.
3. Use {Close_fridge} to close the door firmly.
4. End with {Task_Completed}.

DRAWER RULE:
1. Use {Open_drawer} to pull the drawer open.
2. Pick up the target item with {pickup}.
3. Use {Close_drawer} after the item is removed or replaced.
4. End with {Task_Completed}.

GROCERY UNPACK RULE:
1. Reach into the bag and grip a rigid box or can with {pickup}.
2. Lift straight up to clear the bag rim.
3. Place it neatly on a shelf with {goto_coordinate} and {keep}.
4. End with {Task_Completed}.

DROPPED OBJECT RECOVERY RULE:
1. If an item drops, stop the routine.
2. Locate the new position and move to it.
3. Use {Recover_dropped} or {pickup} to re-grasp.
4. End with {Task_Completed}.

TABLE SETTING RULE:
1. Spread the placemat first.
2. Place the plate in the center of the mat.
3. Place the cup in the top-right corner.
4. Use {Set_table} to represent the full sequence.
5. End with {Task_Completed}.

BOOK SHELF RULE:
1. Hold the book vertically and align it with the shelf opening.
2. Slide it forward into the slot until it is flush.
3. Use {Shelve_book} to place the book.
4. End with {Task_Completed}.

CHAIR PUSH RULE:
1. Place the end-effector against the chair backrest.
2. Apply steady forward force to slide the chair under the table.
3. Use {Push_chair} to complete the motion.
4. End with {Task_Completed}.

Training guidance:
- If you are unsure how to translate a task, follow the examples above.
- Use only the commands listed above.
- Never output any text outside the curly-brace commands.
